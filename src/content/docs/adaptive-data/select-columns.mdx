...
The same `universal_prompt` value is available through the [`datasets.adapt` API](/api/python/resources/datasets/methods/adapt) and Python SDK, so app and programmatic runs can use the same mapping.
...
See **ColumnMapping** on the [`datasets.adapt` reference](/api/python/resources/datasets/methods/adapt) for the current schema.
