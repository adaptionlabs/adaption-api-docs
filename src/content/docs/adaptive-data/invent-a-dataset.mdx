...
`language_expansion` uses the same structure as the [expansion setting saved on a dataset](/adaptive-data/configure-adaptive-data#expansion-saved-on-the-dataset) in Adaptive Data. It expands the invented rows inside the same generation job:
