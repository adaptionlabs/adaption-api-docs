...
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| `learning_rate`        | Peak learning rate for the optimizer. Controls the step size during weight updates.                                                                                                    | Float in `[1e-8, 0.01]`                     |
| `n_epochs`             | Number of complete passes training makes over the dataset.                                                                                                                             | Number, `1`–`20`                            |
| `batch_size`           | Number of samples per training step. Use `"max"` to let the platform select the largest supported batch.                                                                               | `"max"` or integer ≥ 1                      |
| `lr_scheduler_type`    | How the learning rate changes during training. A linear or cosine schedule decays over time; a constant schedule holds it steady.                                                      | `"linear"` \| `"cosine"` \| `"constant"`    |
...
| `scheduler_num_cycles` | Number or fraction of cosine decay cycles across training. Only affects cosine schedules.                                                                                              | Float, `0.0`–`4.0` (default `0.5`)          |
...
| Hyperparameter           | Definition                                                                                                                                              | Valid Values                                                           |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `lora_r`                 | Rank of the LoRA adapter matrices. Controls adapter capacity — higher ranks can represent more complex adaptations but require more memory and compute. | Number, `1`–`64`                                                       |
| `lora_alpha`             | Scaling factor for the LoRA adapter's contribution to the base model. Must be `1×` or `2×` the value of `lora_r`.                                       | Number (`1×` or `2× lora_r`)                                           |
