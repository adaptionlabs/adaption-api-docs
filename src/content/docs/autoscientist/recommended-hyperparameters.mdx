...
`training_method` is a top-level `create` argument (`sft` or `dpo`). The remaining fields below are keys inside `hyperparams`.
...
The recommendation endpoint does not currently generate these DPO-specific values, but `autoscientist.create` accepts them as overrides.
